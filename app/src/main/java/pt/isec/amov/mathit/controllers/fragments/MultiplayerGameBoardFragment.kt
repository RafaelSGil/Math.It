...
class MultiplayerGameBoardFragment : Fragment(R.layout.game_board), View.OnTouchListener {
    enum class MODE{
        HOST,
        CLIENT
    }
    private lateinit var binding : GameBoardBinding
    private var mode: MODE = MODE.HOST
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (i != null) {
            ...
            val modeString= i.getSerializableExtra("mode") as String
            mode = when(modeString) {
                "host" -> MODE.HOST
                "client" -> MODE.CLIENT
                else -> MODE.HOST
            }
            Log.i("DEBUG-AMOV", "onCreate: MODE = $mode")
        }
    ...
    private fun assignLevelToView(){
        if(ConnectionManager.levelData == null)
            return
        ...
        if(ConnectionManager.levelData!!.tables.size<=0)
            manager.goGameOverState(contextActivity, manager)
        val table = ConnectionManager.levelData!!.tables[0]
        ConnectionManager.levelData!!.tables.removeAt(0)

        for(i in 0 until table.cells.size){
            tvs[i].text = table.cells[i]
            Log.i("DEBUG-AMOV", "assignLevelToView: ${tvs[i].text}")
        }
    ...
    private fun generateMultiplayerTables(){
        var cellCounter = 0
        ...
        val levelData = LevelData(level.toString().toInt(), tableList)
        ConnectionManager.sendLevelDataToPlayers(levelData)
        Log.i("DEBUG-AMOV", "generateMultiplayerTables: generated levels")
    }
